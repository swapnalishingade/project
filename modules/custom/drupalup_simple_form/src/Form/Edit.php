...
$data = $query->select('news','n')->fields('n',['id','newstitle','newsdesc','date'])->condition('n.id',$id,'=')->execute()->fetchAll(\PDO::FETCH_OBJ);
//print_r($data);
...
$form['newstitle'] = [
	  ...
	  '#default_value'=>$data[0]->newstitle
...
'#required' => TRUE,
  ...
  $id= \Drupal::routeMatch()->getParameter('id');
   ...
   $postdata=$form_state->getValues();
    ...
    $query = \Drupal::database();
   $query->update('news')->fields($postdata)->condition('id',$id)->execute();
   $response = new \Symfony\Component\HttpFoundation\RedirectResponse('../');
   $response->send();
    // Redirect to home
	  drupal_set_message(" Data Succesfully Updated");
